import { describe, test, expect } from 'vitest';
import { generateSchedule } from './scheduler';

// Pomocnicza — zbierz pracujące godziny per osoba per dzień
function getPersonHoursPerDay(result: Awaited<ReturnType<typeof generateSchedule>>, personId: number, day: number): number[] {
  const hours: number[] = [];
  for (const shift of result.shifts) {
    if (shift.personId === personId && shift.day === day) {
      for (let h = shift.startHour; h < shift.endHour; h++) {
        hours.push(h);
      }
    }
  }
  return hours.sort((a, b) => a - b);
}

/** Helper: uniform params for N people */
function uniformParams(peopleCount: number, hoursPerShift: number, minBreakHours: number) {
  return {
    perPersonShiftHours: Array(peopleCount).fill(hoursPerShift),
    perPersonMinBreak: Array(peopleCount).fill(minBreakHours),
  };
}

describe('scheduler — MIP (HiGHS)', () => {
  test('3 osoby, 8h praca, 3 doby, 8h przerwy → valid, każda osoba codziennie', async () => {
    const result = await generateSchedule({
      peopleCount: 3,
      totalHours: 24 * 3,
      ...uniformParams(3, 8, 8),
    });
    expect(result.valid).toBe(true);
    expect(result.coverageGaps).toHaveLength(0);

    for (let day = 1; day <= 3; day++) {
      for (let p = 0; p < 3; p++) {
        const hours = getPersonHoursPerDay(result, p, day);
        expect(hours.length).toBeGreaterThanOrEqual(1);
      }
    }
  });

  test('10 osób, 8h praca, 3 doby, 8h przerwy → valid, każda osoba codziennie', async () => {
    const result = await generateSchedule({
      peopleCount: 10,
      totalHours: 24 * 3,
      ...uniformParams(10, 8, 8),
    });
    expect(result.valid).toBe(true);
    expect(result.coverageGaps).toHaveLength(0);

    for (let day = 1; day <= 3; day++) {
      for (let p = 0; p < 10; p++) {
        const hours = getPersonHoursPerDay(result, p, day);
        expect(hours.length).toBeGreaterThanOrEqual(1);
      }
    }
  });

  test('każda osoba pracuje max hoursPerShift godzin per zmianę (między przerwami)', async () => {
    const hoursPerShift = 8;
    const minBreakHours = 8;
    const result = await generateSchedule({
      peopleCount: 4,
      totalHours: 24 * 3,
      ...uniformParams(4, hoursPerShift, minBreakHours),
    });

    for (let p = 0; p < 4; p++) {
      const allHours: number[] = [];
      for (const shift of result.shifts) {
        if (shift.personId === p) {
          for (let h = shift.startHour; h < shift.endHour; h++) {
            allHours.push((shift.day - 1) * 24 + h);
          }
        }
      }
      allHours.sort((a, b) => a - b);

      let accumulated = 0;
      let lastEnd = -Infinity;
      for (const hour of allHours) {
        if (hour - lastEnd >= minBreakHours) {
          accumulated = 0;
        }
        accumulated++;
        lastEnd = hour + 1;
        expect(accumulated).toBeLessThanOrEqual(hoursPerShift);
      }
    }
  });

  test('po hoursPerShift godzinach pracy, przerwa >= minBreakHours', async () => {
    const hoursPerShift = 8;
    const minBreakHours = 8;
    const result = await generateSchedule({
      peopleCount: 4,
      totalHours: 24 * 3,
      ...uniformParams(4, hoursPerShift, minBreakHours),
    });
    expect(result.valid).toBe(true);

    for (let p = 0; p < 4; p++) {
      const allHours: number[] = [];
      for (const shift of result.shifts) {
        if (shift.personId === p) {
          for (let h = shift.startHour; h < shift.endHour; h++) {
            allHours.push((shift.day - 1) * 24 + h);
          }
        }
      }
      allHours.sort((a, b) => a - b);

      let accumulated = 0;
      let lastEnd = -Infinity;
      for (const hour of allHours) {
        if (hour - lastEnd >= minBreakHours) {
          accumulated = 0;
        }
        accumulated++;
        lastEnd = hour + 1;
        expect(accumulated).toBeLessThanOrEqual(hoursPerShift);
      }
    }
  });

  test('pokrycie: każda godzina każdego dnia ma co najmniej 1 osobę', async () => {
    const result = await generateSchedule({
      peopleCount: 4,
      totalHours: 24 * 2,
      ...uniformParams(4, 8, 8),
    });
    expect(result.valid).toBe(true);
    expect(result.coverageGaps).toHaveLength(0);

    for (let day = 1; day <= 2; day++) {
      for (let h = 0; h < 24; h++) {
        const hasWorker = result.shifts.some(
          (s) => s.day === day && s.startHour <= h && s.endHour > h
        );
        expect(hasWorker).toBe(true);
      }
    }
  });

  test('praca może być rozbita — nie musi być ciągła', async () => {
    const result = await generateSchedule({
      peopleCount: 4,
      totalHours: 24 * 2,
      ...uniformParams(4, 8, 8),
    });
    expect(result.valid).toBe(true);
    expect(result.shifts.length).toBeGreaterThan(0);
  });

  test('2 osoby, 12h praca, 2 doby, 12h przerwy → valid', async () => {
    const result = await generateSchedule({
      peopleCount: 2,
      totalHours: 24 * 2,
      ...uniformParams(2, 12, 12),
    });
    expect(result.valid).toBe(true);
    expect(result.coverageGaps).toHaveLength(0);
  });

  test('generowanie nazw osób', async () => {
    const result = await generateSchedule({
      peopleCount: 4,
      totalHours: 24 * 1,
      ...uniformParams(4, 8, 8),
      names: ['Jan', 'Anna'],
    });

    const names = result.stats.map((s) => s.personName);
    expect(names).toContain('Jan');
    expect(names).toContain('Anna');
    expect(names).toContain('Osoba 3');
    expect(names).toContain('Osoba 4');
  });

  // --- Testy constraintów ---

  test('limit nocny: max 1 osoba w nocy (22-6)', async () => {
    const result = await generateSchedule({
      peopleCount: 4,
      totalHours: 24 * 2,
      ...uniformParams(4, 8, 8),
      constraints: [
        { type: 'nightShiftLimit', maxPeople: 1, nightStartHour: 22, nightEndHour: 6 },
      ],
    });

    for (let day = 1; day <= 2; day++) {
      for (let h = 0; h < 24; h++) {
        if (h >= 22 || h < 6) {
          const workingNow = result.shifts.filter(
            (s) => s.day === day && s.startHour <= h && s.endHour > h
          );
          expect(workingNow.length).toBeLessThanOrEqual(1);
        }
      }
    }
  });

  test('osoba zablokowana w godzinach 0-8 nie pracuje 0-8', async () => {
    const result = await generateSchedule({
      peopleCount: 3,
      totalHours: 24 * 2,
      ...uniformParams(3, 8, 8),
      constraints: [
        { type: 'personBlocked', personId: 0, startHour: 0, endHour: 8 },
      ],
    });

    for (let day = 1; day <= 2; day++) {
      const hours = getPersonHoursPerDay(result, 0, day);
      for (const h of hours) {
        expect(h).toBeGreaterThanOrEqual(8);
      }
    }
  });

  test('osoba zablokowana nocą (22-6) nie pracuje w tych godzinach', async () => {
    const result = await generateSchedule({
      peopleCount: 4,
      totalHours: 24 * 2,
      ...uniformParams(4, 8, 8),
      constraints: [
        { type: 'personBlocked', personId: 1, startHour: 22, endHour: 6 },
      ],
    });

    for (let day = 1; day <= 2; day++) {
      const hours = getPersonHoursPerDay(result, 1, day);
      for (const h of hours) {
        expect(h >= 6 && h < 22).toBe(true);
      }
    }
  });

  test('zbyt długa przerwa → valid: false', async () => {
    const result = await generateSchedule({
      peopleCount: 2,
      totalHours: 24 * 3,
      ...uniformParams(2, 12, 24),
    });
    expect(result.valid).toBe(false);
    expect(result.errors.length).toBeGreaterThan(0);
  });

  // --- Per-person params ---

  test('per-person shift durations: each shift respects per-person limit', async () => {
    const result = await generateSchedule({
      peopleCount: 3,
      totalHours: 24 * 2,
      perPersonShiftHours: [6, 8, 10],
      perPersonMinBreak: [6, 8, 10],
    });

    const shiftLimits = [6, 8, 10];
    const breakLimits = [6, 8, 10];
    for (let p = 0; p < 3; p++) {
      const allHours: number[] = [];
      for (const shift of result.shifts) {
        if (shift.personId === p) {
          for (let h = shift.startHour; h < shift.endHour; h++) {
            allHours.push((shift.day - 1) * 24 + h);
          }
        }
      }
      allHours.sort((a, b) => a - b);

      let accumulated = 0;
      let lastEnd = -Infinity;
      for (const hour of allHours) {
        if (hour - lastEnd >= breakLimits[p]) {
          accumulated = 0;
        }
        accumulated++;
        lastEnd = hour + 1;
        expect(accumulated).toBeLessThanOrEqual(shiftLimits[p]);
      }
    }
  });

  // --- Start hour offset ---

  test('startHourOffset: schedule starts at hour 14, no work before 14:00 on day 1', async () => {
    const result = await generateSchedule({
      peopleCount: 3,
      totalHours: 48,
      startHourOffset: 14,
      ...uniformParams(3, 8, 8),
    });

    // Day 1 should only have work from 14:00 onwards
    for (let p = 0; p < 3; p++) {
      const hours = getPersonHoursPerDay(result, p, 1);
      for (const h of hours) {
        expect(h).toBeGreaterThanOrEqual(14);
      }
    }

    // Day 3 should only have work before 14:00
    for (let p = 0; p < 3; p++) {
      const hours = getPersonHoursPerDay(result, p, 3);
      for (const h of hours) {
        expect(h).toBeLessThan(14);
      }
    }
  });

  test('daily cap: no person works more than shiftHours per calendar day', async () => {
    const result = await generateSchedule({
      peopleCount: 10,
      totalHours: 24 * 7,
      startHourOffset: 14,
      ...uniformParams(10, 8, 11),
      constraints: [],
    });

    expect(result.valid).toBe(true);

    // For each person, count work hours per calendar day
    const totalDays = Math.ceil((14 + 24 * 7) / 24);
    for (let p = 0; p < 10; p++) {
      for (let d = 1; d <= totalDays; d++) {
        const hours = getPersonHoursPerDay(result, p, d);
        expect(hours.length).toBeLessThanOrEqual(8);
      }
    }
  });

  test('daily cap with night shifts: night workers respect daily limit', async () => {
    const result = await generateSchedule({
      peopleCount: 10,
      totalHours: 24 * 5,
      startHourOffset: 14,
      ...uniformParams(10, 8, 11),
      constraints: [
        { type: 'nightShiftLimit', maxPeople: 1, nightStartHour: 22, nightEndHour: 6 },
        ...Array.from({ length: 8 }, (_, i) => ({
          type: 'personBlocked' as const, personId: i, startHour: 22, endHour: 6,
        })),
      ],
    });

    expect(result.valid).toBe(true);

    const totalDays = Math.ceil((14 + 24 * 5) / 24);
    for (let p = 0; p < 10; p++) {
      for (let d = 1; d <= totalDays; d++) {
        const hours = getPersonHoursPerDay(result, p, d);
        expect(hours.length).toBeLessThanOrEqual(8);
      }
    }
  });

  // === TESTY DLA 6 OSÓB ===

  test('6 osób, 8h praca, 3 doby, 11h przerwy → valid, pełne pokrycie', async () => {
    const result = await generateSchedule({
      peopleCount: 6,
      totalHours: 24 * 3,
      ...uniformParams(6, 8, 11),
    });
    expect(result.valid).toBe(true);
    expect(result.coverageGaps).toHaveLength(0);
    expect(result.errors).toHaveLength(0);

    // Każda godzina ma co najmniej 1 osobę
    for (let day = 1; day <= 3; day++) {
      for (let h = 0; h < 24; h++) {
        const hasWorker = result.shifts.some(
          (s) => s.day === day && s.startHour <= h && s.endHour > h
        );
        expect(hasWorker).toBe(true);
      }
    }
  });

  test('6 osób, 8h praca, 7 dni, 11h przerwy → valid, sprawiedliwy rozkład', async () => {
    const result = await generateSchedule({
      peopleCount: 6,
      totalHours: 24 * 7,
      ...uniformParams(6, 8, 11),
    });
    expect(result.valid).toBe(true);
    expect(result.coverageGaps).toHaveLength(0);

    // Sprawdź sprawiedliwy rozkład godzin — rozpiętość max 16h
    const hours = result.stats.map(s => s.totalWorkHours);
    const maxDiff = Math.max(...hours) - Math.min(...hours);
    expect(maxDiff).toBeLessThanOrEqual(16);
  });

  test('6 osób, 8h praca, 3 doby — przerwy >= 11h między zmianami', async () => {
    const minBreakHours = 11;
    const result = await generateSchedule({
      peopleCount: 6,
      totalHours: 24 * 3,
      ...uniformParams(6, 8, minBreakHours),
    });
    expect(result.valid).toBe(true);

    // Sprawdź przerwy
    for (let p = 0; p < 6; p++) {
      const allHours: number[] = [];
      for (const shift of result.shifts) {
        if (shift.personId === p) {
          for (let h = shift.startHour; h < shift.endHour; h++) {
            allHours.push((shift.day - 1) * 24 + h);
          }
        }
      }
      allHours.sort((a, b) => a - b);

      // Znajdź przerwy między blokami pracy
      for (let i = 1; i < allHours.length; i++) {
        const gap = allHours[i] - allHours[i - 1];
        if (gap > 1) {
          // To jest przerwa
          expect(gap).toBeGreaterThanOrEqual(minBreakHours);
        }
      }
    }
  });

  test('6 osób, daily cap: max 8h pracy na dzień kalendarzowy', async () => {
    const result = await generateSchedule({
      peopleCount: 6,
      totalHours: 24 * 3,
      ...uniformParams(6, 8, 11),
    });
    expect(result.valid).toBe(true);

    for (let p = 0; p < 6; p++) {
      for (let d = 1; d <= 3; d++) {
        const hours = getPersonHoursPerDay(result, p, d);
        expect(hours.length).toBeLessThanOrEqual(8);
      }
    }
  });

  test('6 osób z limitem nocnym: max 2 osoby w nocy (22-6)', async () => {
    const result = await generateSchedule({
      peopleCount: 6,
      totalHours: 24 * 3,
      ...uniformParams(6, 8, 11),
      constraints: [
        { type: 'nightShiftLimit', maxPeople: 2, nightStartHour: 22, nightEndHour: 6 },
      ],
    });
    expect(result.valid).toBe(true);
    expect(result.coverageGaps).toHaveLength(0);

    for (let day = 1; day <= 3; day++) {
      for (let h = 0; h < 24; h++) {
        if (h >= 22 || h < 6) {
          const workingNow = result.shifts.filter(
            (s) => s.day === day && s.startHour <= h && s.endHour > h
          );
          expect(workingNow.length).toBeLessThanOrEqual(2);
        }
      }
    }
  });

  test('6 osób, różne długości zmian i przerw', async () => {
    const result = await generateSchedule({
      peopleCount: 6,
      totalHours: 24 * 3,
      perPersonShiftHours: [6, 8, 8, 10, 12, 6],
      perPersonMinBreak: [6, 8, 11, 10, 12, 6],
    });

    // Powinno się udać wygenerować
    expect(result.coverageGaps).toHaveLength(0);

    // Sprawdź limity zmian per osoba
    const shiftLimits = [6, 8, 8, 10, 12, 6];
    const breakLimits = [6, 8, 11, 10, 12, 6];
    for (let p = 0; p < 6; p++) {
      const allHours: number[] = [];
      for (const shift of result.shifts) {
        if (shift.personId === p) {
          for (let h = shift.startHour; h < shift.endHour; h++) {
            allHours.push((shift.day - 1) * 24 + h);
          }
        }
      }
      allHours.sort((a, b) => a - b);

      let accumulated = 0;
      let lastEnd = -Infinity;
      for (const hour of allHours) {
        if (hour - lastEnd >= breakLimits[p]) {
          accumulated = 0;
        }
        accumulated++;
        lastEnd = hour + 1;
        expect(accumulated).toBeLessThanOrEqual(shiftLimits[p]);
      }
    }
  });

  test('6 osób, startHourOffset=14, 5 dni', async () => {
    const result = await generateSchedule({
      peopleCount: 6,
      totalHours: 24 * 5,
      startHourOffset: 14,
      ...uniformParams(6, 8, 11),
    });
    expect(result.valid).toBe(true);
    expect(result.coverageGaps).toHaveLength(0);

    // Day 1 only has work from 14:00 onwards
    for (let p = 0; p < 6; p++) {
      const hours = getPersonHoursPerDay(result, p, 1);
      for (const h of hours) {
        expect(h).toBeGreaterThanOrEqual(14);
      }
    }
  });

  test('6 osób z 2 zablokowanymi nocą — nadal pełne pokrycie', async () => {
    const result = await generateSchedule({
      peopleCount: 6,
      totalHours: 24 * 3,
      ...uniformParams(6, 8, 11),
      constraints: [
        { type: 'personBlocked', personId: 0, startHour: 22, endHour: 6 },
        { type: 'personBlocked', personId: 1, startHour: 22, endHour: 6 },
      ],
    });
    expect(result.valid).toBe(true);
    expect(result.coverageGaps).toHaveLength(0);

    // Osoby 0 i 1 nie pracują w nocy
    for (let day = 1; day <= 3; day++) {
      for (const blockedPerson of [0, 1]) {
        const hours = getPersonHoursPerDay(result, blockedPerson, day);
        for (const h of hours) {
          expect(h >= 6 && h < 22).toBe(true);
        }
      }
    }
  });

  test('partial last day: everyone gets exactly expected hours, no excess', async () => {
    const result = await generateSchedule({
      peopleCount: 4,
      totalHours: 91,
      startHourOffset: 2,
      ...uniformParams(4, 8, 11),
      constraints: [
        { type: 'nightShiftLimit', maxPeople: 1, nightStartHour: 0, nightEndHour: 8 },
      ],
    });

    // No coverage gaps
    expect(result.coverageGaps).toHaveLength(0);

    // Each person should work exactly 32h (4 shifts × 8h), not more
    for (let p = 0; p < 4; p++) {
      const hours = result.stats[p].totalWorkHours;
      expect(hours).toBe(32);
    }
  });
});
